...
export const calculatePuppyAgeUtil = (birthDate: Date, dict: Dictionary): string => {
    const now = new Date();
    ...
    if (years === 1) {
      if (remainingMonths === 0) {
        ...
        return replaceText(dict.admin.utils.age.yearAndMonths, { count: remainingMonths });
      }
    }
    ...
    if (remainingMonths === 0) {
      ...
      return replaceText(dict.admin.utils.age.yearsAndMonths, { years, months: remainingMonths });
    }
  };